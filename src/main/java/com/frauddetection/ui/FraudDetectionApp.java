...
public class FraudDetectionApp extends Application {
    ...
    private VBox createTransactionPane() {
        ...
        transactionTable.getColumns().addAll(timeCol, idCol, accountCol, symbolCol, sideCol, quantityCol, priceCol, totalCol);
    ...
    private HBox createStatusBar() {
        ...
        Label timestamp = new Label("Last Update: " + LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss")));
                ...
                for (ConsumerRecord<String, String> record : records) {
                    try {
                        ...
                        Platform.runLater(() -> {
                            ...
                            alertCountLabel.setStyle("-fx-font-size: 16px; -fx-text-fill: #e74c3c; -fx-background-color: #f8d7da; -fx-padding: 2px 6px; -fx-background-radius: 3px;");
    ...
    public static class FraudAlertDisplayModel {
        ...
        public FraudAlertDisplayModel(FraudAlert alert) {
            ...
            this.riskScore = alert.getRiskScore().multiply(new BigDecimal("100")).setScale(1, java.math.RoundingMode.HALF_UP) + "%";
